...
function terragruntPlanAll {
  # Gather the output of `terragrunt plan-all`.
  ...
  if [ ${planExitCode} -eq 2 ]; then
    ...
    if echo "${planOutput}" | egrep '^-{72}$' &> /dev/null; then
        planOutput=$(echo "${planOutput}" | sed -n -r '/Terraform will perform/,/-{72}/{/-{72}/d;/Terraform will perform/d; p}')
    fi
  ...
  if [ ${planExitCode} -ne 0 ]; then
    echo "plan-all: error: failed to plan Terragrunt configuration in ${tfWorkingDir}"
    echo "${planOutput}"
    echo
  fi
